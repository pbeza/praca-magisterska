...
\note[itemize]{
	\item Oprogramowanie maszyn \textbf{nie musi być identyczne} --- parametryzowanie szablonami.
	\item Problem \textbf{dotyczy głównie administratorów}, ale też ludzi używających wielu podobnie skonfigurowanych komputerów.
	\item \textbf{Miejsca zastosowania:} laboratoria na~uczelniach, szkołach, bibliotekach, urzędach, sklepach, grupy serwerów, biletomaty, bankomaty, automaty sprzedające (maszyny \emph{vendingowe}).
	\item \textbf{,,Ręczne'' metody radzenia sobie z~problemem} bez wykorzystania dedykowanego oprogramowania: ,,ręczna'' instalacja gotowego obrazu systemu na każdym z~komputerów z~osobna, SSH, zdalny pulpit~(VNC).
}
...
\begin{exampleblock}{}
	...
	\vskip2mm
	\hspace*\fill{\small--- Formalnie zgłoszona tematyka pracy magisterskiej}
\end{exampleblock}

\begin{figure}
	\includegraphics[height=.3\textheight]{img/client_server}
\end{figure}

\end{frame}

\note[itemize]{
	\item \textbf{Wbrew nazwie ,,protokół''} głównym celem było \textbf{zaimplementowanie aplikacji ,,serwera'', która umożliwia przygotowanie i~dostosowanie obrazu zmian konfiguracji systemu wzorcowego}, który można zastosować na~systemie klienckim za~pomocą aplikacji ,,klienckiej'', \textbf{a~nie samo przesłanie takiego obrazu do~klienta} (co~zostało zaimplementowane za~pomocą \texttt{SFTP}).
	\item \textbf{Aplikacja serwera = aplikacja dla~system będący wzorcem oprogramowania dla~klientów}, a~nie klasyczny model komunikacji klient-serwer. Przyjęty model komunikacji działa raczej w~modelu \emph{peer2peer}.
	\item Zaimplementowane \textbf{rozwiązanie było testowane na}~dystrybucjach \textbf{\texttt{Debian}} i~\textbf{\texttt{ArchLinux}}, ale~jest \textbf{napisane w~Python} i~prawdopodobnie działa również na~innych.
}
...
\note[itemize]{
	\item Rozwiązaniem przedstawionego problemu są~systemy nazywane \textbf{systemami SCM, czyli \emph{Software Configuration Systems}} --- nie mylić z~oprogramowaniem typu \emph{Source Control Management}, takimi jak np.~\texttt{git} i~\texttt{svn}.
	\item \textbf{4~najpopularniejsze wg~rankingu Debiana} systemy zarządzania konfiguracją.
	\item \textbf{Najstarszy i~najpopularniejszy} wg~rankingu jest \textbf{Puppet}. Znani użytkownicy: \textbf{CERN, Google, NASA, Intel}.
	\item Wszystkie napisane w~\textbf{językach skryptowych} (Python, Ruby, Erlang).
	\item Wszystkie używają \textbf{deklaratywnej konfiguracji} (a \textbf{nie imperatywnej}), która opisuje \textbf{oczekiwany stan systemu}, a~\textbf{nie kroki jakie trzeba podjąć}, aby go uzyskać.
	\item \textbf{Puppet pierwszy pod~względem wyrafinowania i~popularności}, ponieważ \textbf{buduje graf zależności zasobów} i~na~jego podstawie decyduje w~jakiej kolejności zmieniać konfigurację systemu.
}
...
\note[itemize]{
	\item \textbf{Cel ten sam} co~istniejących systemów SCM, ale~\textbf{zupełnie inne podejście} --- zamiast deklaratywnie/imperatywnie opisywać oczekiwany stan systemu wystarczy skonfigurować maszynę wzorcową.
	\item Użycie polega na~\textbf{naprzemiennym skanowaniu i~modyfikowaniu} systemu wzorcowego.
	\item \textbf{Wygenerowanie obrazu} zmian jest \textbf{możliwe po~przeskanowaniu} systemu.
	\item Obrazy zmian są~\textbf{podpisane cyfrowo} przez system wzorcowy.
	\item \textbf{Klienci mogą pobrać} obraz zmian od~systemu wzorcowego lub od siebie nawzajem.
}

%------------------------------------------------------------------------------

\begin{frame}{Zalety i~wady alternatywnego rozwiązania}

\begin{columns}[t]
	\begin{column}{0.5\textwidth}
		\textbf{Zalety rozwiązania:}
		\begin{itemize}
			\item[\done] Łatwość użytkowania
			\item[\done] Rozproszenie
			\item[\done] Parametryzacja konfiguracji
			\item[\done] Przenośność na~inne dystrybucje \texttt{GNU/Linux}
			\item[\done] Bezpieczeństwo
			\item[\done] Elastyczność
		\end{itemize}
	\end{column}
	\begin{column}{0.5\textwidth}
		\textbf{Dalsze kierunki rozwoju:}
		\begin{itemize}
			\item[\wontfix] Wykorzystanie multicastu
			\item[\wontfix] Obsługa ,,kłopotliwych'' sytuacji (GRUB)
			\item[\wontfix] Raportowanie przez klientów
			\item[\wontfix] Obsługa np.~Windows
			%\item[\wontfix] Tworzenie obrazów zmian na~podstawie \textbf{dowolnych dwóch wyników skanowań}
			%\item[\wontfix] Lepsza obsługa sytuacji awaryjnych --- np.~nagłe przerwanie procesu zastosowania obrazu zmian powinno być tranzakcyjne (wszystko albo nic)
		\end{itemize}
	\end{column}
\end{columns}
...
\note[itemize]{
	...
	\item \textbf{Elastyczność} --- możliwość synchronizacji wybranej części systemu.
}
...
%			\item niesynchronizowanie \textbf{\path{/var/lib/lightdm}}
%		\end{itemize}
%\end{itemize}
%
%\end{frame}
