...
Cele niniejszej pracy przedstawione w~rozdziale~\ref{sec:cel-i-zakres} zostały zrealizowane --- w~ramach pracy zostało zaprojektowane i~zaimplementowane rozwiązanie umożliwiające propagowanie zmian w~systemie plików, pakietów oraz~elementów konfiguracji do~stacji roboczych pod~kontrolą systemu operacyjnego \glslink{gnulinux}{Linux/GNU}. Ponadto na~początku pracy przedstawiono przegląd istniejących rozwiązań omawianego w~tej pracy problemu dystrybuowania oprogramowania i~skonfrontowano je z~projektem zaimplementowanym w~ramach niniejszej pracy. Na~końcu pracy przedstawiono możliwe dalsze kierunki rozwoju aplikacji czyniące ją~wydajniejszą na~etapie propagacji zmian przygotowanych przez zaimplementowane oprogramowanie.

Najważniejszą cechą odróżniającą opisane istniejące rozwiązania od~autorskiego rozwiązania jest sposób działania --- administrator komputerowy zamiast szczegółowo opisywać oczekiwany stan systemu stacji klienckich, może z~skonfigurować system ,,wzorcowy'' w~sposób jaki chciałby skonfigurować pozostałe maszyny klienckie, a~następnie może rozpropagować wprowadzone zmiany na~komputery klienckie. Takie alternatywne podejście do~problemu wydaje się być proste, jednak istnieje wiele technicznych uwarunkowań i~niuansów działania systemu operacyjnego, które istotnie utrudniają realizację implementacji według przyjętego scenariusza ,,klonowania'' stacji wzorcowej. Istnienie plików \emph{cache} wspomnianych w~rozdziale~\ref{sec:duze-testy} to~tylko jeden z~wielu przykładów trudności technicznych i~projektowych z~jakimi należy się zmierzyć próbując przenieść wzorzec konfiguracji z~jednego systemu na~drugi. Innym przykładem problematycznego scenariusza dla~zaproponowanego rozwiązania mogłaby być operacja aktualizacji jądra systemu operacyjnego (kernela), której standardowo towarzyszy wygenerowanie konfiguracji dla~programu rozruchowego (np.~dla~GRUBa), która zawiera takie dane jak np.~nazwy partycji systemu zawierające jądro systemu, identyfikatory sprzętowe i~inne unikalne dane, które byłoby trudno obsłużyć w~przyjętym modelu działania zaimplementowanych aplikacji \texttt{\srvappname{}} i~\texttt{\cliappname{}}. Brak istniejących rozwiązań działających w~sposób podobny do~zaimplementowanego\footnote{Według [nie]wiedzy autora.} być może świadczy o~trudności lub~złożoności realizacji zaproponowanego w~ramach tej pracy pomysłu. Pomysł ten może być szczególnie kłopotliwy w~przypadku chęci synchronizowania oprogramowania na~dwóch różnych dystrybucjach systemów.
