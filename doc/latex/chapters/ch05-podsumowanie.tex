...
\begin{itemize}
	\item \hrefemph{https://tex.stackexchange.com/q/368367/44382}{\texttt{\textbackslash{}adjustbox} with XeLaTeX misplaces hyperlinks} --- problem z~błędnie wyświetlanymi niektórymi hiperłączami po~kompilacji z~wykorzystaniem \href{https://www.sharelatex.com/learn/XeLaTeX}{XeLaTeX} za~pomocą polecenia \texttt{latexmk -pdf -pvc -pdflatex=xelatex} (\texttt{\url{https://tex.stackexchange.com/q/368367/44382}}),
...
Pomimo napotkanych i~potencjalnych problemów związanych z~przyjętym pomysłem implementacji, nie sposób nie docenić zalet zaproponowanego rozwiązania. Administrator zamiast poświęcać czas najpierw na~naukę języka metakonfiguracji właściwej konfiguracji systemów klienckich, a~następnie na~iteracyjnym dopracowywaniu takiej metakonfiguracji, może skupić się na~docelowej konfiguracji, którą chce przenieść na~systemy klienckie. Zaletę tę~chyba łatwiej docenić gdy~samemu spróbuje się poprawnie skonfigurować jedno z~istniejących rozwiązań w~nietrywialnym przypadku (tzn.~np.~na~rzeczywistym serwerze) i~policzy się czas jaki to~zajęło lub~gdy~zobaczy się jak wiele istnieje różnych rodzajów stron internetowych, list mailingowych i~poradników poświęconych rozwiązywaniu problemów administratorów mających różnego rodzaju problemy z~wyrażeniem pożądanej konfiguracji w~języku danego rozwiązania, np.~w~manifestach \hyperref[sec:puppet]{Puppet} czy~przepisach \hyperref[sec:chef]{Chef}.
