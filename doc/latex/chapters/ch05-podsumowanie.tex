...
W~czasie prac nad~implementacją niniejszego projektu napotkano na~wiele (głównie małych, ale~uciążliwych) technicznych problemów, wątpliwości i~niuansów, których tylko niewielka część znalazła odzwierciedlenie w~zadanych przez autora kilku pytaniach na~forum \hreftt{https://stackoverflow.com/}{stackoverflow.com}. Poniżej zaprezentowano krótką listę kilku takich przykładowych problemów wraz z~krótkim wyjaśnieniem o~co~w~nim chodziło i~z~adresem do~strony internetowej do~danego problemu i~jego rozwiązania:
\begin{itemize}
...
Cele niniejszej pracy przedstawione w~rozdziale~\ref{sec:cel-i-zakres} zostały zrealizowane --- zostało zaprojektowane i~zaimplementowane rozwiązanie umożliwiające propagowanie zmian w~systemie plików, pakietów oraz~elementów konfiguracji do~stacji roboczych pod~kontrolą systemu operacyjnego \glslink{gnulinux}{Linux/GNU}. Ponadto na~początku pracy przybliżono istniejące rozwiązania problemu dystrybuowania oprogramowania i~skonfrontowano je z~zaimplementowanym projektem. Na~końcu pracy opisano dalsze możliwe kierunki rozwoju aplikacji \texttt{\srvappname{}} dla~stacji wzorcowej, potencjalnie czyniące ją~wydajniejszą w~propagacji obrazów zmian konfiguracji dla~klientów.
