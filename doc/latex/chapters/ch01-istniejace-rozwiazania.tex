...
Puppet składa się~z~kilku narzędzi pomocniczych --- jednym z~nich jest \hreftt{https://docs.puppet.com/facter/}{facter}, który na~żądanie zbiera informacje o~systemie i~wyświetla je w~jednym z~możliwych formatów: \json{}, \hreftt{https://en.wikipedia.org/wiki/YAML}{YAML} lub~\hreftt{https://en.wikipedia.org/wiki/Plaintext}{plaintext}, który jest formatem domyślnym. Wywołanie polecenia \texttt{facter} bez dodatkowych parametrów, wyświetla szczegóły dotyczące systemu operacyjnego.

Opiekunowie projektu \href{https://puppet.com/download-learning-vm}{przygotowali} maszynę wirtualną z~samouczkiem tak, aby nowi użytkownicy Puppeta nie~byli skazani na~jego naukę na~własnym systemie~\cite{puppet-vm-tutorial}. W~przypadku błędnej konfiguracji Puppeta, stan maszyny wirtualnej z~samouczkiem może być~łatwo przywrócony, co~często nie jest proste w~przypadku nieudanych eksperymentów na~niewirtualizowanym systemie.
...
Powyższe sposoby uruchamiania Ansible są~użyciem Ansible~\emph{ad-hoc}. Możliwości aplikacji ujawniają się w~pełni, gdy~wykorzystamy \hrefemph{http://docs.ansible.com/playbooks_intro.html}{playbook}, czyli odpowiednik manifestu Puppeta i~przepisu Chefa.
...
Salt, nazywany też \emph{SaltStack}, to~otwartoźródłowe oprogramowanie napisane w~Pythonie przez Amerykanina --- \href{https://www.linkedin.com/in/thhatch/}{Thomasa Hatcha} --- i~opublikowane na~warunkach licencji \glslink{apache2.0-license}{Apache~2.0}. Funkcjonalność Salt jest podobna do~dotychczas opisanych rozwiązań. Pierwsza wersja Salt powstała w~2011~roku, a~więc jest to~najmłodsze rozwiązanie z~dotychczas przedstawionych~\cite{saltstack}.
...
\end{center}
zastosuje plik stanu \path{nettools.sls} dla~agentów pracujących pod~kontrolą dystrybucji \href{https://en.wikipedia.org/wiki/Ubuntu_(operating_system)}{Ubuntu}, których nazwa pasuje do~wyrażenia regularnego \texttt{minion*} lub~których adres~IP pasuje do~wyrażenia regularnego~\texttt{192.168.50.*}.
...
\newcommand{\saltpricing}{\href{https://saltstack.com/saltstack-enterprise/}{Nie podano}}%tablefootnote{Cena zostaje podana po~kontakcie przez e-mail \hreftt{mailto:sales@saltstack.com}{sales@saltstack.com}.}}
...
\begin{itemize}
	...
	\item\label{itm:cfengine} \emph{\cfengine{}} (skrót od~ang.~\emph{The~\textbf{C}on\textbf{f}iguration \textbf{E}ngine}) --- otwartoźródłowy~(opublikowany na~licencji~\glslink{gpl}{GPL}) system zarządzania konfiguracją autorstwa \href{https://en.wikipedia.org/wiki/Mark_Burgess_(computer_scientist)}{prof.~Marka Burgessa} z~Uniwersytetu w~Oslo~\cite{cfengine}. Projekt jest najstarszy z~całego zestawienia --- narodził się bowiem w~1993~roku w~Departamencie Fizyki Teoretycznej, gdzie pracował autor projektu. W~roku~2009 została ogłoszona jego komercyjna wersja. \cfengine{} działał pierwotnie tylko na~systemach~\glslink{unix-like-system}{*nixowych}, ale~obecnie wspiera również Windows. \cfengine{} wyróżnia się na~tle konkurencji tym, że~jako jedyny z~zestawienia ma~podstawy teoretyczne stworzone specjalnie na~jego potrzeby (\hrefemph{https://en.wikipedia.org/wiki/Promise\_theory}{promise theory}~\cite{cfengine-whitepaper,mark-burgess-webpage}) oraz tym, że~został napisany w~języku~C, a~nie języku skryptowym lub~innym języku wyższego poziomu jak pozostałe rozwiązania. Rozpoznawalnym użytkownikiem CFEngine jest \hrefemph{https://www.linkedin.com/feed/}{LinkedIn}\footnote{LinkedIn używa lub~używał również oprogramowania \hyperref[sec:salt]{Salt}~\cite{salt-linkedin}.} --- międzynarodowy serwis społecznościowy, specjalizujący się w~kontaktach zawodowo-biznesowych kupiony w~2016~roku przez \hrefemph{https://en.wikipedia.org/wiki/Microsoft}{Microsoft}~\cite{cfengine-linkedin}.
...
\begin{table}[t]
	...
	\begin{tabular}{l|l|l|l|l}
		...
		Nazwa konfiguracji      & \emph{manifest}                       & \emph{recipe} / \emph{cookbook}   & \emph{playbook}                      & \emph{state file}                \\\hline
