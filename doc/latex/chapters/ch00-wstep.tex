...
\begin{lstlisting}[numbers=none,caption={Przykładowa konfiguracja klientów serwera Ansible, w~nomenklaturze Ansible nazywana \emph{Playbook}},label=ansible-playbook-example]
---
- hosts: web
  vars:
...
\begin{enumerate}
	...
	\item Aplikacji napisanej w~języku Python oraz oraz~towarzyszącej jej dokumentacji wygenerowanej z~komentarzy w~kodzie źródłowym.
\end{enumerate}
...
%		\item dokumentacja biblioteki libconfig~\cite{libconfig-doc}.
	...
	\end{enumerate}
\end{enumerate}
Kompletny spis odwołań cytowanych w~niniejszej dokumentacji, znajduje się~na~jej \hyperref[bibliography-page]{końcu}.
