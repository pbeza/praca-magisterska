...
Głównym celem niniejszej pracy jest opracowanie i~implementacja protokołu komunikacji serwera i~klienta, który pozwala na~wydajne i~bezpieczne przekazanie przez serwer klientom ich oczekiwanego stanu konfiguracji, a~następnie umożliwia klientom zastosowanie się do~tej konfiguracji, aby~osiągnęły stan oczekiwany przez serwer. Takie deklaratywne podejście do~problemu dystrybycji oprogramowania, które opisuje~\emph{co}, a~nie~\emph{jak}, klienci mają zmienić w~swojej konfiguracji, aby~dostosować się do~wymagań postawionych przez administratora sieci komputerowej, jest nazywane określeniem \hrefemph{https://en.wikipedia.org/wiki/Infrastructure_as_Code}{Infrastructure as~Code~(IaC)}.
...
PGM~specyfikuje mechanizmy minimalizacji przekazywanych duplikatów \texttt{NAK}~w~górę drzewa oraz~procedurę, dzięki której pakiety naprawcze \texttt{RDATA} nie są~przekazywane do~tych części sieci, z~których nie były wysyłane pakiety~\texttt{NAK}. PGM~może bez utraty funkcjonalności (ale~z~częściową stratą wydajności) działać w~sieciach, w~których część lub~żaden router nie implementuje tego protokołu. W~przypadku korzystania z~protokołu~IP jako warstwy sieciowej, ze~względu na~konieczność warunkowego przetwarzania pakietów~\texttt{SPM}, \texttt{NCF} i~\texttt{RDATA}, muszą one mie mieć ustawioną flagę \hrefemph{https://tools.ietf.org/html/rfc2113}{IP~Router Alert}, która wymusza na~routerach ich specjalne potraktowanie.
...
\begin{enumerate}
	...
	\item\hypertarget{itm:server-role-2} serwer buforujący pakiety oprogramowanie (\hrefemph{https://wiki.archlinux.org/index.php/Package_Proxy_Cache}{Package Proxy Cache}) na~żądanie klientów dostarcza im~unicastem oprogramowanie opisane w~konfiguracji otrzymanej multicastem; serwer ten~pozwala administratorowi nadpisanie domyślnej konfiguracji wybranych pakietów,
...
\begin{figure}
	...
	\caption{Przykładowa sieć działająca pod~kontrolą serwerów dystrbucji oprogramowania}
