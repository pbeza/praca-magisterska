...
{
	name={daemon},
	description={Program komputerowy działający w~tle. Nazwy \glslink{daemon}{daemonów} w~systemach \glslink{unix-like-system}{*niksowych} zwyczajowo kończą się literą~\textbf{d}}
}
\newglossaryentry{rfc}
{
	name={RFC},
...
{
	name={RSA},
	description={\emph{Rivest-Shamir-Adleman} Asymetryczny algorytm kryptograficzny z~kluczem publicznym, zaprojektowany w~1977r. przez \emph{Rona Rivesta}, \emph{Adi Szamira} i \emph{Leonarda Adlemana}~\cite{wiki:rsa,rsa}. Był to~pierwszy algorytm, który może być stosowany zarówno do szyfrowania jak i~podpisów cyfrowych}
}
\newglossaryentry{dh}
{
	name={Diffie-Hellman},
	description={Algorytm pozwalający dwóm stronom na~ustalenie tajnego klucza w~niezabezpieczonej sieci~\cite{mimuw-ssl-w04}. Efemeryczna wersja tego protokołu jest używana w~celu osiągnięcia własności \glslink{pfs}{Perfect Forward Secrecy}}
}
\newglossaryentry{pfs}
{
	name={PFS},
