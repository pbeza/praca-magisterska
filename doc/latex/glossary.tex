...
{
	name={Unix},
	description={Rodzina systemów operacyjnych, mających wspólny początek w~postaci systemu \gls{unix} powstałego w~1969~roku w~\emph{AT\&T} (obecnie \emph{Bell~Labs}). Korzystając z~kodu \glslink{unix}{Unix'a} powstało wiele pokrewnych systemów operacyjnych, m.in.~BSD, Solaris, Sun~OS, System~V, AIX, HP-UX, Mac~OS~X. Projekt \gls{gnu} był od~początku projektowany jako ulepszony \gls{unix}, nie bazujący na~jego kodzie źródłowym~\cite{gnu-initial-msg}.}
}
...
\newglossaryentry{copyleft}
{
	...
	description={Licencje umożliwiające uczynienie programu \glslink{wolne-oprogramowanie}{wolnym oprogramowaniem} i~nakazanie, by~wszystkie jego zmienione i~poszerzone wersje również były takimi~\cite{copyleft,wiki:licenses-comparison}.}
}
\newglossaryentry{ssh}
{
	name={SSH},
...
{
	name={RSA},
	description={\emph{Rivest-Shamir-Adleman} Asymetryczny algorytm kryptograficzny z~kluczem publicznym, zaprojektowany w~1977~roku przez Rona Rivesta, Adi Szamira i~Leonarda Adlemana~\cite{wiki:rsa,rsa}. Był to~pierwszy algorytm, który może być stosowany zarówno do~szyfrowania jak i~podpisów cyfrowych.}
}
\newglossaryentry{dh}
{
	name={Diffie-Hellman},
...
{
	name={OpenSSL},
	description={Popularna, wieloplatformowa biblioteka kryptograficzna napisana w~języku~\texttt{C}, opublikowana na~licencji typu \glslink{copyleft}{copyleft}~\cite{openssl}.}
}
